...
/// }
/// ```
pub struct PickListWithMenuWidth<
    'a,
    T,
    L,
    V,
...
impl<'a, T, L, V, Message, Theme, Renderer>
    PickListWithMenuWidth<'a, T, L, V, Message, Theme, Renderer>
where
    T: ToString + PartialEq + Clone,
    L: Borrow<[T]> + 'a,
    V: Borrow<T> + 'a,
    ...
    Renderer: text::Renderer,
{
    /// Creates a new [`PickListWithMenuWidth`] with the given list of options, the current
    ...
    /// Sets the placeholder of the [`PickListWithMenuWidth`].
    pub fn placeholder(mut self, placeholder: impl Into<String>) -> Self {
        self.placeholder = Some(placeholder.into());
        self
    }

    /// Sets the width of the [`PickListWithMenuWidth`].
    pub fn width(mut self, width: impl Into<Length>) -> Self {
        self.width = width.into();
        self
    }

    /// Sets the menu width of the [`PickListWithMenuWidth`].
    pub fn menu_width(mut self, width: f32) -> Self {
        self.menu_width = Some(width);
        self
    }

    /// Sets the [`Padding`] of the [`PickListWithMenuWidth`].
    pub fn padding<P: Into<Padding>>(mut self, padding: P) -> Self {
        self.padding = padding.into();
        self
    }

    /// Sets the text size of the [`PickListWithMenuWidth`].
    pub fn text_size(mut self, size: impl Into<Pixels>) -> Self {
        self.text_size = Some(size.into());
        self
    }

    /// Sets the text [`text::LineHeight`] of the [`PickListWithMenuWidth`].
    pub fn text_line_height(mut self, line_height: impl Into<text::LineHeight>) -> Self {
        self.text_line_height = line_height.into();
        self
    }

    /// Sets the [`text::Shaping`] strategy of the [`PickListWithMenuWidth`].
    pub fn text_shaping(mut self, shaping: text::Shaping) -> Self {
        self.text_shaping = shaping;
        self
    }

    /// Sets the font of the [`PickListWithMenuWidth`].
    pub fn font(mut self, font: impl Into<Renderer::Font>) -> Self {
        self.font = Some(font.into());
        self
    }

    /// Sets the [`Handle`] of the [`PickListWithMenuWidth`].
    pub fn handle(mut self, handle: Handle<Renderer::Font>) -> Self {
        self.handle = handle;
        self
    }

    /// Sets the message that will be produced when the [`PickListWithMenuWidth`] is opened.
    pub fn on_open(mut self, on_open: Message) -> Self {
        self.on_open = Some(on_open);
        self
    }

    /// Sets the message that will be produced when the [`PickListWithMenuWidth`] is closed.
    pub fn on_close(mut self, on_close: Message) -> Self {
        self.on_close = Some(on_close);
        self
    }

    /// Sets the style of the [`PickListWithMenuWidth`].
    #[must_use]
...
impl<'a, T, L, V, Message, Theme, Renderer> Widget<Message, Theme, Renderer>
    for PickListWithMenuWidth<'a, T, L, V, Message, Theme, Renderer>
where
    ...
    Renderer: text::Renderer + 'a,
{
    ...
    fn overlay<'b>(
        ...
        if state.is_open {
            ...
            .width(self.menu_width.unwrap_or(bounds.width))
...
impl<'a, T, L, V, Message, Theme, Renderer>
    From<PickListWithMenuWidth<'a, T, L, V, Message, Theme, Renderer>>
...
where
    ...
    Renderer: text::Renderer + 'a,
{
    fn from(pick_list: PickListWithMenuWidth<'a, T, L, V, Message, Theme, Renderer>) -> Self {
        Self::new(pick_list)
    }
}
...
impl<P: text::Paragraph> State<P> {
    /// Creates a new [`State`] for a [`PickListWithMenuWidth`].
    fn new() -> Self {
        Self {
...
/// The handle to the right side of the [`PickListWithMenuWidth`].
...
pub enum Handle<Font> {
    ...
    Dynamic {
        /// The [`Icon`] used when [`PickListWithMenuWidth`] is closed.
        closed: Icon<Font>,
        /// The [`Icon`] used when [`PickListWithMenuWidth`] is open.
        open: Icon<Font>,
    },
...
/// The possible status of a [`PickListWithMenuWidth`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Status {
    /// The [`PickListWithMenuWidth`] can be interacted with.
    Active,
    /// The [`PickListWithMenuWidth`] is being hovered.
    Hovered,
    /// The [`PickListWithMenuWidth`] is open.
    Opened {
        /// Whether the [`PickListWithMenuWidth`] is hovered, while open.
        is_hovered: bool,
    },
}
...
/// The theme catalog of a [`PickListWithMenuWidth`].
pub trait Catalog: menu::Catalog {
    ...
    /// The default class for the menu of the [`PickListWithMenuWidth`].
...
/// A styling function for a [`PickListWithMenuWidth`].
///
...
/// The default style of the field of a [`PickListWithMenuWidth`].
